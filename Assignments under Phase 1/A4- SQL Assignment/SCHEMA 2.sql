...
CREATE TABLE locations (
    location_id SERIAL PRIMARY KEY, -- SERIAL replaces AUTO_INCREMENT
...
-- -----------------------------------------------------
-- 3. DATA INSERTION (Converted to PostgreSQL Date Format)
-- -----------------------------------------------------
INSERT INTO regions
VALUES (
	1,
...
COMMIT;
-- -----------------------------------------------------
-- 4. VIEW CREATION (Joining all tables)
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- 5. ESTABLISHING RELATIONSHIPS (Foreign Keys)
-- -----------------------------------------------------
...
SELECT DISTINCT first_name, last_name, email, phone_number FROM employees;
...
-- 12. Print a bonafide certificate for an employee (say for emp. id 123) as below: "This is to certify that <full name> with employee id <emp. id> is working as <job id> in dept. <dept ID>. (1 ROW) [NOTE : EMPLOYEES table].

SELECT 'This is to certify that ' || first_name || ' ' || last_name || 
       ' with employee id ' || employee_id || ' is working as ' || job_id || 
       ' in dept. ' || department_id || '.' AS bonafide_certificate
FROM employees 
WHERE employee_id = 123;

-- 13. Write a query to display the employee id, salary & salary range of employees as 'Tier1', 'Tier2' or 'Tier3' as per the range <5000, 5000-10000, >10000 respectively, ordering the output by those tiers.(107 ROWS)[NOTE :EMPLOYEES TABLE]
...
AND e1.manager_id = (SELECT manager_id FROM departments WHERE department_name = 'Accounting');
...
-- 19. Write a query in SQL to display the name of the department, average salary and number of employees working in that department who got commission. (One row)
